...
pub fn update(best_guess: &str, pattern: [Tile; 5], answers: Vec<String>) -> Vec<String> {
    
    let new_answers: Vec<String> = answers.into_iter()
        .filter(|word| {
            // We 'borrow' the word for get_pattern
            get_pattern(best_guess, word) == pattern 
        })
        .collect();

    new_answers // Return the new owned Vec
}
