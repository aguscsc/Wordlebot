...
def entropy(answers, guess):
    ...
    print(
        f"the best guess is {best_guess}, giving {max_entropy} bits of information on avg"
    )
    return best_guess
...
def main(answers, guess, total_answers):
    best_guess = input(
        "enter your first word (you should start with the word raise, it gives ~5.87bits of information): "
    )
    while 1:
        ...
        print(
            f"this answer gave you {actual_information} bits of information, {len(answers)} words remain\n"
        )
        if len(answers) <= 1:
            break
